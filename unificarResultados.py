...
with open(output_path, 'w', encoding='utf-8') as result_file:
    ...
    result_file.write(f"Incorrectas de tipo 1 (Generacion de palabras con otro part of speech. la palabra a analizar no está como sustantivo en la frase.): {combined_values['incorrectas_1']} ({combined_percentages['incorrectas_1']:.2f}%)\n")
    result_file.write(f"Incorrectas de tipo 2 (la palabra a analizar no aparece en la frase): {combined_values['incorrectas_2']} ({combined_percentages['incorrectas_2']:.2f}%)\n")
